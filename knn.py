from cupy import sort, where, array, zeros, argmax, bincount
...
class KNN:
    ...
    def get_nearest_k(self, X_test):
        ...
        # K번째로 작은 수
        KminusOne = (sort(self.distance_matrix)[self.K - 1])
        # 가장작은 k개의 인덱스 번호
        self.idx = where(self.distance_matrix <= KminusOne)
        
    def majority_vote(self):
        # label 배열로 변환
        label = bincount(self.y[self.idx], minlength = self.y_name.shape[0])
#         print(label)
        # 가장 많은 label 구하기
        Max = argmax(label)
        # 꽃 이름?
...
#         yarr = self.y[self.idx]       
        yarr = array(self.y)[self.idx].tolist()

        # 가중치들의 합을 계산
        result = zeros(self.y_name.shape[0])
        for i in range(self.K):
            result[yarr[i]] += arr[i]
        # 가중치 합이 가장 큰 값의 꽃 이름?    
#         print(result)
        return self.y_name[argmax(result)]
